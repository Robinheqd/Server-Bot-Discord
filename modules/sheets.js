...
// const GoogleSpreadsheet = require("./google-spreadsheet_fork");

const SPREADSHEET_KEY = process.env.SPREADSHEET_URL.substr(
  39,
  process.env.SPREADSHEET_URL.length
).split("/")[0];
      ...
      if (err) {
        ...
        if (sheetData.worksheets) {
          for (var i in sheetData.worksheets) {
            sheetData.worksheets[i].apiURL =
              API_URL + sheetData.worksheets[i].title;
          }
        }
        resolve(sheetData);
      }
...
// function addData(tab, message) {
//   // return new Promise(function(resolve,reject) {

//   getInfo(SPREADSHEET_KEY, API_URL).then(function(info) {
...
//       }
//     };
//     sheet.addRow(test, function(err, row) {
//       if (err) {
//         console.log("error in sheetjs  ", err);
//         // reject(err.error);
//       } else {
//         //resolve(row)
//       }
